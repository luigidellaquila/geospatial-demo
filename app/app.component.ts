...
export class AppComponent implements OnInit {
  ...
  drawMap() {
    var controller = this;
    let mapProp = {
      center: new google.maps.LatLng(50.10902, 14.5831653),
      zoom: 16,
  ...
  createPerson(): void {
    var location = {"@class": "OPoint", coordinates: [this.lon, this.lat]}
  ...
  createEdge(from: any, to: any): void {
    this.orient.command(`create edge FriendOf from ${from.rid} to ${to.rid}`, (x)=> {
      console.log(x)
    }, (x)=> {
      console.log(x)
    })
